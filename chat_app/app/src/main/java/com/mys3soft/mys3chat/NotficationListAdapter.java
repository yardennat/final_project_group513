        ...
        btn.setOnClickListener(
                new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        ...
                        map1.put("score","0");
                        fireBase.child(user.getEmail()).child(friendEmail).setValue(map1);

                        Map<String, String> map2 = new HashMap<>();
                        map2.put("Email", user.getEmail());
                        map2.put("FirstName", user.getFirstName());
                        map2.put("LastName", user.getLastName());
                        map2.put("score",String.valueOf(user.getScore()));
                        fireBase.child(friendEmail).child(user.getEmail()).setValue(map2);

                        Firebase frRequ = new Firebase(StaticInfo.EndPoint + "/friendrequests");
                        frRequ.child(user.getEmail()).child(friendEmail).removeValue();
                        ...
                        notMap.put("SenderEmail", user.getEmail());
                        notMap.put("FirstName", user.getFirstName());
                        notMap.put("LastName", user.getLastName());
                ...
                new AlertDialog.Builder(con)
                        ...
                        .setPositiveButton("Reject", new DialogInterface.OnClickListener() {
                            ...
                            public void onClick(DialogInterface dialog, int which) {
                                User user = LocalUserService.getLocalUserFromPreferences(con);
                                Firebase fireBase = new Firebase(StaticInfo.FriendRequestsEndPoint + "/" + user.getEmail() + "/" + getItem(modelPosition).FriendRequestFireBaseKey);
