...
public class DataContext extends SQLiteOpenHelper {
    ...
    public DataContext(Context context, String name, SQLiteDatabase.CursorFactory factory, int version) {
        super(context, "chat-example-f10a5.db", factory, 10);
    }
    ...
    public List<User> getUserFriendList() {
        ...
        while (!c.isAfterLast()) {
            try {
                ...
                friend.setEmail( c.getString(c.getColumnIndex("Email")));
                friend.setFirstName(c.getString(c.getColumnIndex("FirstName")));
                friend.setLastName(c.getString(c.getColumnIndex("LastName")));
    ...
    public void refreshUserFriendList(List<User> friendList) {
        ...
        for (User item : friendList) {
            ...
            if (checkFriendAlreadyExists(item.getEmail()) == 0) {
                // insert
                SQLiteDatabase db = getWritableDatabase();
                String query = "insert into Friends (Email,FirstName,LastName) values('" + item.getEmail() + "', '" + item.getFirstName() + "', '" + item.getLastName() + "');";
                db.execSQL(query);
                // db.close();
            }
        }
    ...
    public List<Message> getUserLastChatList(String userMail) {
        ...
        for (User friend : userFriendList) {
            String query = "select rowid, * from Messages where (FromMail = '" + userMail + "' and ToMail='" + friend.getEmail() + "') or (ToMail = '" + userMail + "' and FromMail='" + friend.getEmail() + "')  order by rowid desc limit 1  ";
            Cursor c = db.rawQuery(query, null);
            c.moveToFirst();
            try {
                Message mess = new Message();
                // set from email to friend so when user click on list to navigate to chat activity
                mess.FromMail = friend.getEmail();
                mess.Message = c.getString(c.getColumnIndex("Message"));
                mess.Message = mess.Message.replace("\n", "");
                mess.SentDate = c.getString(c.getColumnIndex("SentDate"));
                mess.FriendFullName = friend.getFirstName() + " " + friend.getLastName();
