...
/**
 * this class is responsible for finding a friend among all users
 */
...
public class ActivityAddContact extends AppCompatActivity {
    ...
    /**
     * this function is working when we click on button search
     */
            ...
            try {
                ...
                for (Iterator iterator = jsonObjectList.keys(); iterator.hasNext(); ) {
                    try {
                        ...
                        String fullName = f.getFirstName().toLowerCase() + " " + f.getLastName().toLowerCase();
                        if (f.getEmail().toLowerCase().contains(serKey) || fullName.contains(serKey)) {
